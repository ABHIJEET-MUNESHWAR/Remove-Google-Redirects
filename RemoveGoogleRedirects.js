...
/*
	Based on script from: http://userscripts.org/scripts/show/121261
	Original License: GPL v3 or later version
	Original Author: Kevin Wang (kevixw'At'gmail.com)
	Script modified by: Abhijeet Ashok Muneshwar
*/
...
var StraightGoogle = (function ($$d) {
	...
	function _start() {
		...
		else if (/:\/\/books\.google\.[\w.]+\w\/.*$/.test($K.url)) {
			$K.debug('Straight Google [Play] is now loaded'); 
			common_clean_job();
		}
		...
		else if (/:\/\/www\.google\.com\/(finance)\/.*$/.test($K.url)) { 
			$K.debug('Straight Google [Finance] is now loaded'); 
			common_clean_job();
		}		
		...
		// Google Image Search
			...
			$K.window.onReady(
				...
				function ($w) {
					try {
						// keyword .H().K
						$w._.Pj.H().K = function () {};	// the redirection URL generator function
						/* keyword
						 ...
						 */
						$w._.AR = function () {};	// reset the redirection function
					}
					catch (e) {
